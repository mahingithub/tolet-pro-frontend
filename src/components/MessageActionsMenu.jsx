...
// ─────────────────────────────────────────────────────────────────────────────
// The message ACTIONS menu (Reply · Forward · Copy · Download · Pin · Remove),
...
//
// Download only shows for photo / video bubbles (parent decides via
// `canDownload`) and saves the file to the device. Mute is deliberately NOT
// here: muting is a whole-conversation setting, so it belongs to the chat
// header / chat-list menus, not a single message.
//
...
//   canDownload           show Download (image / video messages)
//   pinned                toggles the Pin / Unpin label
//   onReply, onForward, onCopy, onDownload, onPin, onDelete, onClose
//   labels                { reply, forward, copy, download, pin, unpin, remove }

import React, { useEffect, useState, useLayoutEffect } from 'react';
import { createPortal } from 'react-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { CornerUpLeft, Forward, Copy, Download, Pin, PinOff, Trash2 } from 'lucide-react';

const MENU_W = 232;

export default function MessageActionsMenu({
  open, x = 0, y = 0, mine = false, canCopy = false, canDownload = false, pinned = false,
  onReply, onForward, onCopy, onDownload, onPin, onDelete, onClose, labels = {},
}) {
  ...
  useLayoutEffect(() => {
    ...
    const rows = 2 + (canCopy ? 1 : 0) + (canDownload && onDownload ? 1 : 0) + (onPin ? 1 : 0) + (onDelete ? 1 : 0);
  ...
  }, [open, x, y, canCopy, canDownload, onDownload, onPin, onDelete]);
      ...
      {open && (
        <>
          ...
          >
            ...
            {canDownload && onDownload && <Item icon={Download} label={labels.download || 'Download'} onClick={onDownload} />}
